...
nombres_filas <- c(35, 75, 125)
nombres_columnas <- c("0.01", "0.05")
...
for (Distribucion in c("Pareto", "Normal")){
  Pos_N = 1
  for (N in c(35,75,125)){
    Pos_error = 1
    for(error in c("001","005")){
    start_time= Sys.time()

# Especifica la ruta del archivo RDS
ruta_archivo_rds <- paste("X_se_Muestras_", Distribucion, "_Monotona_", error, "_", N, ".rds", sep = "")

X_se <- readRDS(ruta_archivo_rds)
y <- read.csv(paste("y_Muestras_", Distribucion, "_Monotona_", error, "_", N, ".csv", sep = ""), dec = ".")
Beta_real <- read.csv(paste("Beta_Muestras_", Distribucion, "_Monotona_", error, "_", N, ".csv", sep = ""), dec = ".")
t <- read.csv(paste("T_Muestras_", Distribucion, "_Monotona_", error, "_", N, ".csv", sep = ""), dec = ".")
Beta_real_2 <- read.csv(paste("Beta2_Muestras_", Distribucion, "_Monotona.csv", sep = ""), dec = ".")
T2 <- read.csv(paste("T2_Muestras_", Distribucion, "_Monotona.csv", sep = ""), dec = ".")
T2 <- as.vector(T2$x)
...
for (i in 1:n_sims) {
  ...
  cv<-cv.SOFR.con(Ngrid,y_sim,X_sim,T,constr = "dec")
  indmin<-which.min(cv)
  ###############################################
  N1 <- Ngrid[indmin] #selected N1

  out<-SOFR.con(N1,y_sim,X_sim,T,T2 = T2,m,constr = "dec")
...
for (i in 1:n_sims) {
  ...
  n<-length(y)
  Ngrid<-c(3:8)  ###grid for choosing N1
  cv<-cv.SOFR.con(Ngrid,y_sim,X_sim,T,constr = "inc")
  indmin<-which.min(cv)
  ###############################################
  N1 <- Ngrid[indmin] #selected N1

  out<-SOFR.con(N1,y_sim,X_sim,T,T2 = T2,m,constr = "inc")
  # out.NULL<-SOFR.NULL(N1,y,X,T,m,constr = NULL)
  beta1t<-out$betaSOFR #constrained estimate
  betaIMSE <- out$betaIMSE
...
df <- data.frame(x = 1:length(results_constrained$mean),
                 mean = results_constrained$mean,
                 lower = results_constrained$lower,
                 upper = results_constrained$upper,
                 Beta_real = Beta_real$x)

# Dibujar el gráfico
plot(df$x, df$mean, type = "l", col = "blue", lty = 2, ylim = range(c(df$lower, df$upper, df$Beta_real)),
    xlab = "Índice", ylab = "Valor", main = "Estimación con restricciones")
polygon(c(df$x, rev(df$x)), c(df$lower, rev(df$upper)), col = "pink", border = NA)
lines(df$x, df$Beta_real, col = "green", lty = 2)
lines(df$x, df$mean, col = "blue", lty = 2)
abline(h = 0, col = "red", lty = 2)
legend("top", legend = c("Mean", "CI", "Real Beta", "Zero Line"), col = c("blue", "pink", "green", "red"), 
       lty = c(1, NA, 2, 2), pch = c(NA, 15, NA, NA), bty = "n")

if(Distribucion == "Pareto"){
  IMSE_Restricciones_Pareto[Pos_N,Pos_error] = mean(IMSE_cons)*100
  IMSE_sd_Restricciones_Pareto[Pos_N,Pos_error] = sd(IMSE_cons)
  Tiempo_Restricciones_Pareto[Pos_N,Pos_error] = tiempo_constrained
}else{
  IMSE_Restricciones_Normal[Pos_N,Pos_error] = mean(IMSE_cons)*100
  IMSE_sd_Restricciones_Normal[Pos_N,Pos_error] = sd(IMSE_cons)
...
# Dibujar el gráfico
plot(df$x, df$mean, type = "l", col = "blue", lty = 2, ylim = range(c(df$lower, df$upper, df$Beta_real)),
    xlab = "Índice", ylab = "Valor", main = "Estimación sin restricciones")
polygon(c(df$x, rev(df$x)), c(df$lower, rev(df$upper)), col = "pink", border = NA)
lines(df$x, df$Beta_real, col = "green", lty = 2)
lines(df$x, df$mean, col = "blue", lty = 2)
abline(h = 0, col = "red", lty = 2)
...
if(Distribucion == "Pareto"){
  IMSE_NORestricciones_Pareto[Pos_N,Pos_error] = mean(IMSE_uncons_2)*1000
  IMSE_sd_NORestricciones_Pareto[Pos_N,Pos_error] = sd(IMSE_uncons_2)*1000
  Tiempo_NORestricciones_Pareto[Pos_N,Pos_error] = tiempo_unconstrained
}else{
  IMSE_NORestricciones_Normal[Pos_N,Pos_error] = mean(IMSE_uncons_2)*1000
  IMSE_sd_NORestricciones_Normal[Pos_N,Pos_error] = sd(IMSE_uncons_2)*1000
...
```{r}
  knitr::kable(IMSE_Restricciones_Pareto)
  knitr::kable(IMSE_sd_Restricciones_Pareto)
  knitr::kable(Tiempo_Restricciones_Pareto)
  knitr::kable(IMSE_NORestricciones_Pareto)
  knitr::kable(IMSE_sd_NORestricciones_Pareto)
  knitr::kable(Tiempo_NORestricciones_Pareto)
