...
class Fun_IntConstraints:
    ...
    def _get_matrices_W(self) -> List[np.ndarray]:
        ...
        for i in range(bsp.matrixB.shape[1] - 1 - bsp.deg):
    ...
    def _create_PSD_var(self, model: mosek.fusion.Model) -> mosek.fusion.PSDVariable:
        ...
        n = [bsp.matrixB.shape[1] - 1 - bsp.deg for bsp in self.bspline.values()]
    ...
    def interval_cons(
        ...
        for w in range(
            self.bspline[self.var_name].matrixB.shape[1] - 1
